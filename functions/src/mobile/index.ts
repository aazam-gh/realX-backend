...
const recordOnlineVendorClick = async (uid: string, vendorId: string, requestId: string) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const tracked = shouldTrackOnlineClick(throttleDoc.data()?.count || 0);
...
const processTransaction = async ({
  ...
  const rewardAccountRef = db.collection('rewardAccounts').doc(uid);
  ...
  return db.runTransaction(async (tx) => {
    ...
    tx.set(rewardAccountRef, {
      userId: uid,
      statusPoints: admin.firestore.FieldValue.increment(100),
